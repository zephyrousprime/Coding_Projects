...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (title && homeCards.length) {
        ...
        animate(title, titlesar[0].anime[0]);

        animate(homeCards, titlesar[0].anime[1]);

        if (overCard) {
            animate(overCard, titlesar[0].anime[2]);
        }
    }
    ...
    document.addEventListener('titlechange', (e) => {
        ...
        const interval = setInterval(() => {
            ...
            for (let i = 0; i < newText.length; i++) {
                if (newText[i] === ' ') {
                    displayed += ' ';
                } else if (i < Math.floor((step / stepbob) * newText.length)) {
                    displayed += newText[i];
                } else {
    ...
    document.addEventListener('themeclick', () => {
        ...
        if (!btn) return;
        animate(btn, titlesar[0].anime[3]);
    });
        ...
        const chartObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    animate(entry.target, titlesar[0].anime[4]);
    ...
    document.addEventListener('ai-stream-start', () => {
        ...
        if (!btn) return;
        aiPulseAnim = animate(btn, titlesar[0].anime[5]);
    });
    ...
    document.addEventListener('ai-stream-end', () => {
        ...
        if (btn) {
            animate(btn, titlesar[0].anime[6]);
        }
    });
});
